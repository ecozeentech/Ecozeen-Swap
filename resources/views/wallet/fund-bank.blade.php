<x-app-layout>
    ...
    <div class="max-w-lg mx-auto rounded-2xl border border-charcoal-100 dark:border-charcoal-800 bg-white dark:bg-charcoal-900 p-6 shadow-sm">
        ...
        @if (! empty($transaction->metadata['receiving_wallet_address'] ?? null))
            <div class="mt-4 rounded-xl bg-brand-50 dark:bg-charcoal-800 border border-brand-100 dark:border-charcoal-700 p-4 text-sm">
                <p class="text-charcoal-400 text-xs mb-1">Your purchase will be sent to</p>
                <p class="font-mono font-semibold break-all">{{ $transaction->metadata['receiving_wallet_address'] }}</p>
            </div>
        @endif
