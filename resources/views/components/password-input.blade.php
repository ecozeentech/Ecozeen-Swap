...
<div x-data="{ show: false }" class="relative">
    <input
        ...
        {{ $attributes->merge(['id' => $id, 'name' => $name, 'autocomplete' => $autocomplete, 'class' => 'block mt-1 w-full rounded-lg border-charcoal-200 pr-10 shadow-sm focus:border-brand-500 focus:ring-brand-500 dark:bg-charcoal-800 dark:border-charcoal-600 dark:text-white']) }}
    />
    <button
        type="button"
        @click="show = !show"
        tabindex="-1"
        class="absolute inset-y-0 right-0 top-1 flex items-center px-3 text-charcoal-400 hover:text-brand-500 focus:outline-none"
