            ...
            <div class="rounded-2xl border border-charcoal-100 dark:border-charcoal-800 bg-white dark:bg-charcoal-900 p-5 shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @foreach ($wallets as $wallet)
                        @php $logoModel = $wallet->isCrypto() ? $cryptoAssets->firstWhere('symbol', $wallet->currency_code) : $fiatCurrencies->firstWhere('code', $wallet->currency_code); @endphp
                        <div class="rounded-xl bg-charcoal-50 dark:bg-charcoal-800 px-4 py-3">
                            <div class="flex items-center gap-2">
                                @if ($logoModel)
                                    <img src="{{ $logoModel->logoUrl() }}" class="h-5 w-5 rounded-full object-cover" alt="">
                                @endif
                                <p class="text-xs text-charcoal-400">{{ $wallet->currency_code }}</p>
                            </div>
                            <p class="font-bold text-charcoal-900 dark:text-white mt-1">{{ number_format($wallet->balance, $wallet->isCrypto() ? 6 : 2) }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
